  ...
  initialRender2P() {
    ...
    for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 8; j++) {
        ...
        if ((i % 2) === 0) {
          if ((j % 2) === 0) {
            chessBoardDisplay.innerHTML += `<div class="papaya-whip square"   onclick="clickEventBoard2P(${i},${j})"></div>`

          }
          else {
            chessBoardDisplay.innerHTML += `<div class="green square" onclick="clickEventBoard2P(${i},${j})"></div>`
          }
        }
        else {
          if ((j % 2) === 0) {
